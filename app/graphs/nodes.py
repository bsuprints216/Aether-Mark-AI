...
def trend_analyzer_node(state: SocialState) -> Dict[str, Any]:
    if MOCK_MODE:
        return {
            "trend_data": "AetherMark AI Simulated Trends: #AI_Orchestration, #AutonomousMarketing, #AgenticWorkflows",
...
def content_generator_node(state: SocialState) -> Dict[str, Any]:
    if MOCK_MODE:
        content = "AetherMark AI Simulated Content: 1. Strategic Post on Cognitive Orchestration. 2. Insights on Autonomous ROI."
...
def campaign_scheduler_node(state: SocialState) -> Dict[str, Any]:
    if MOCK_MODE:
        return {
            "scheduling_status": "AetherMark AI Execution: All cognitive assets queued for synchronized distribution.",
            "next_step": "__end__"
        }
...
def engagement_responder_node(state: SocialState) -> Dict[str, Any]:
    if MOCK_MODE:
        return {
            "messages": state.get("messages", []) + ["AetherMark AI Responder: Handled 5 distinct community interactions."],
            "next_step": "__end__"
        }
    factory = AgentFactory(state["brand_profile"].model_dump())
...
def analytics_reporter_node(state: SocialState) -> Dict[str, Any]:
    if MOCK_MODE:
        return {
            "analytics_report": "AetherMark AI Intelligence: Reach +24.2%, Cognitive Engagement +8.5%.",
            "next_step": "__end__"
        }
    factory = AgentFactory(state["brand_profile"].model_dump())
